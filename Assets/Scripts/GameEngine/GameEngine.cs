...
public class GameEngine
{
    ...
    public void PlayCard(int playerID, int cardInstanceID)
    {
        ...
        if (cardToPlay != null)
        {
            ...
            PendingCardInstance = cardToPlay;
            Debug.Log("游戏阶段切换为" + State.CurrentPhase + 
    ...
    public void MindbugDecision(int playerID, bool useMindbug)
    {
        ...
        {
            // 假设使用Mindbug
            State.Players[State.ActivePlayerID].Hand.Remove(PendingCardInstance);
            State.Players[State.ExpectedPlayerID].Field.Add(PendingCardInstance);
            State.Players[State.ExpectedPlayerID].MindbugCount-=1;
            Debug.Log("玩家" + playerID + "使用了Mindbug，卡牌" 
            + PendingCardInstance.CardData.CardName + "加入到玩家" + State.ExpectedPlayerID + "的场上");
        ...
        else
        {
            ...
            State.Players[State.ActivePlayerID].Hand.Remove(PendingCardInstance);
            State.Players[State.ActivePlayerID].Field.Add(PendingCardInstance);
            ChangeGamePhase(GamePhase.WaitingForMainAction);
